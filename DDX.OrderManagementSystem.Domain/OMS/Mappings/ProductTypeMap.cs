...
namespace DDX.OrderManagementSystem.Domain
{
    ...
    public class ProductTypeMap : ClassMap<ProductType>
    {
        public ProductTypeMap()
        {
            ...
            Map(x => x.Enabled);
            Map(x => x.Caigou1).Length(2000);
            Map(x => x.Caigou2).Length(2000);
            Map(x => x.Caigou3).Length(2000);
            Map(x => x.Caigou4).Length(2000);
            Map(x => x.ProductAttribute).Length(100);
            Map(x => x.FromTo).Length(100);
            Map(x => x.Suppliers).Length(200);
            Map(x => x.HSCode);
            Map(x => x.MUnit);
            Map(x => x.IsOpenInvoice);
            Map(x => x.InvoicePrice);
        }
    }
}
