...
namespace DDX.OrderManagementSystem.App.Controllers
{
    public class EbayMessageController : BaseController
    {
        ...
        public JsonResult ListEbayMessage(int page, int rows, string sort, string order, string search)
        {
            ...
            {
                objList[i].MessageStatus =objList[i].MessageStatus;
                objList[i].MessageType =objList[i].MessageType;
            }
        ...
        public JsonResult GetTree()
        {
            ...
            foreach (DictionaryType item in fristList)
            {
                ...
                {//呈现Ebay消息分类
                    ...
                    tree2.Add(new SystemTree { id = item.FullName+"~"+item2.FullName+"~"+ item2.DicValue, text = item2.FullName});
                }
                //List<SystemTree> tree2 = ConvertToTree(fooList);
                //呈现Ebay邮件分类
                trees.Add(new SystemTree { id = item.FullName + "~~", text = item.FullName, children = tree2 });
            }
            //trees.Add(tree);
            return Json(trees);
        }
