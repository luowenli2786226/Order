...
namespace DDX.OrderManagementSystem.App.Controllers
{
    public class UserController : BaseController
    {
        ...
        public ActionResult Create()
        {
            ...
            u.Code = Utilities.GetNo(NSession, Utilities.UserNo);
            return View(u);
        }
        ...
        public JsonResult Change(string oldpsd, string newpsd)
        {
            try
            {
                if (oldpsd == CurrentUser.Password)
                {
                    ...
                    Utilities.CreateCookies(obj.Username, obj.Password, 1);
                }
                else
        ...
        [HttpPost]
        public ActionResult Login(string u, string p, int t)
        {
            bool iscon = Utilities.LoginByUser(u, p, NSession);
            if (iscon)
            {
                Utilities.CreateCookies(u, p, t);
        ...
        {
            UserType userType = NSession.Get<UserType>(id);
            if (userType != null)
            {
                userType.DeletionStateCode = 1;
                NSession.Update(userType);
                NSession.Flush();
            }
