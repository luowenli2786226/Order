...
namespace DDX.OrderManagementSystem.App
{
    public class EBayUtil
    {
        ...
        public static void EbayUploadTrackCode(string account, DDX.OrderManagementSystem.Domain.OrderType orderType)
        {
            ...
            if (accounts.Count > 0)
            {
                ...
                IList<DDX.OrderManagementSystem.Domain.OrderType> orderList = new List<DDX.OrderManagementSystem.Domain.OrderType>();
                if (orderType.IsMerger == 1 || orderType.OrderExNo.IndexOf("|") != -1)
                {
                    orderList = NSession.CreateQuery("from OrderType where MId='" + orderType.Id + "' Or Id ='" + orderType.Id + "'").List<DDX.OrderManagementSystem.Domain.OrderType>();
                }
                else
                {
                ...
                //CarrierUsed = "China Post";//20161207暂注释
                if (orderType.FBABy == "UKMAN")
                {
                    //CarrierUsed = "Hermes";
                    CarrierUsed = "Yodel";
                }
                ...
                {
                    CarrierUsed = "FedEx";//Ebay 海外仓默认单号上传发货方式为FedEx
                }
                call = new eBay.Service.Call.CompleteSaleCall(context);
                foreach (DDX.OrderManagementSystem.Domain.OrderType order in orderList)
                {
                    string orderid = "";
                    string itemid = "";
        ...
        public static void EbayUploadTrackCode1(string account, DDX.OrderManagementSystem.Domain.OrderType orderType,string  l)
        {
            ISession NSession = NhbHelper.OpenSession();
            Dictionary<string, int> sendNum = new Dictionary<string, int>();
            IList<AccountType> accounts = NSession.CreateQuery("from AccountType where AccountName='" + account + "'").SetMaxResults(1).
                    ...
                    if (order.OrderExNo.IndexOf("-") == -1)
                    {
                        ...
                        if (orders.Count > 0)
                        {
                            foreach (TransactionType trans in orders[0].TransactionArray)
                            {
                                ...
                                call.Shipment.DeliveryStatus = eBay.Service.Core.Soap.ShipmentDeliveryStatusCodeType.Delivered;
                                call.Shipment.ShipmentTrackingDetails = new ShipmentTrackingDetailsTypeCollection();
                                //ShipmentTrackingDetailsType t = new ShipmentTrackingDetailsType();
