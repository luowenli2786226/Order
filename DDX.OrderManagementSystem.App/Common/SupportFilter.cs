...
namespace DDX.OrderManagementSystem.App
{
    public class SupportFilterAttribute : ActionFilterAttribute
    {
        ...
        {
            bool iscon = false;
            if (filterContext.HttpContext.Request.FilePath.IndexOf("User/ValidateCode") != -1 || filterContext.HttpContext.Request.FilePath.IndexOf("User/Login") != -1 || filterContext.HttpContext.Request.FilePath.IndexOf("Home/SaveFile") != -1 || filterContext.HttpContext.Request.FilePath.IndexOf("Home/SavePic") != -1 || filterContext.HttpContext.Request.FilePath.IndexOf("Home/PrintGrf") != -1 || filterContext.HttpContext.Request.FilePath.IndexOf("Home/PrintData") != -1 || filterContext.HttpContext.Request.FilePath.IndexOf("Home/PrintSave") != -1)
            {
                return;
            }
            ...
            if (filterContext.HttpContext.Session["account"] == null)
            {
                ...
                if (!string.IsNullOrEmpty(str))
                {
                    if (str.IndexOf('&') != -1)
                    {
                        ...
                        iscon = Utilities.LoginByUser(strs[0], strs[1], NhbHelper.GetCurrentSession());
        ...
        private static bool IsAuthorization(ActionExecutingContext filterContext, string controller, string action)
        {
            ...
            DDX.OrderManagementSystem.Domain.UserType account = (DDX.OrderManagementSystem.Domain.UserType)filterContext.HttpContext.Session["account"];
